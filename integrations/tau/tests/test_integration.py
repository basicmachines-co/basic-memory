...
async def test_real_basic_memory_roundtrip_and_fresh_recall(
    ...
    try:
        ...
        note = await make_tool(connection, by_name["read_note"]).execute(
            "read",
            {"identifier": receipt.file_path, "project": "main"},
        )
    ...
    # A separate runtime reconnects and recalls the note without an orientation command.
    ...
    try:
        ...
        assert "Tau continuity checkpoint" in recalled
    finally:
