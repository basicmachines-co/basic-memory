...
def search(filters: list[str], project_ref: str | None = None, timeout: int = 10):
...
with ThreadPoolExecutor(max_workers=4 + MAX_SHARED) as pool:
    ...
    fut_codex = pool.submit(search, RECENT_CODEX, primary_project or None)
    fut_sessions = pool.submit(search, RECENT_GENERIC, primary_project or None)
...
if primary_tasks is None and primary_decisions is None and primary_codex is None:
    if not configured:
        ...
        print(
            "# Basic Memory for Codex\n\n"
            f"_Could not read from `{proj}`. Run `Use bm-status` to check the "
...
if codex_rows:
    lines += ["", f"## Recent Codex Checkpoints ({len(codex_rows)})", *[label(r) for r in codex_rows]]
elif session_rows:
    lines += ["", f"## Recent Sessions ({len(session_rows)})", *[label(r) for r in session_rows]]
