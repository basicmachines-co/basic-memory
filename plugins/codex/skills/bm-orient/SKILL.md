...
2. Query the primary project:
   ...
   - recent Codex sessions: `type=codex_session`, after `recallTimeframe`; when
     `sessionProfile=coding`, add `metadata_filters={"repo": repository}`
   - recent core-projected sessions: `type=session`, after `recallTimeframe`

   Always query `codex_session` and `session`. Merge and deduplicate the results, sort them
   newest first, and prefer the highest-signal checkpoint regardless of which
   producer wrote it. Coding-profile `codex_session` notes carry schema-required,
   queryable Git context while preserving one Codex recall shape; `session`
   carries normalized artifacts from `bm hook flush`.
...
4. Read the highest-signal hits before summarizing. Prefer notes that match the
   ...
   path. For coding profiles, filter `codex_session` metadata such as `repo`,
   `branch`, `git_sha`, or `pr` before falling back to text search.
