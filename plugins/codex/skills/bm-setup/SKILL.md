...
repository identifier because the coding CodexSession schema requires queryable
Git identity fields.
...
- `codex-session.md` for a general profile, or `codex-session-coding.md` for a
  coding profile; both define `type: codex_session`, and only one is seeded
- `decision.md`
- `task.md`
...
- Search the primary project for `type=schema` with page size 10. For a coding
  setup, confirm the `Codex Session` schema contains required `repo`, `repo_root`,
  `cwd`, `branch`, and `git_sha` frontmatter fields.
