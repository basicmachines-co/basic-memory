...
async def accept_document_note(
    ...
    if await file_service.exists(path):
        ...
        await require_untouched_raw_projection(
            file_service, existing, path=path, markdown=existing_markdown
        )
...
async def require_untouched_raw_projection(
    ...
    *,
    ...
    Text reads normalize native line endings; YAML comments, formatting and body
    text remain significant so a refresh cannot discard authored changes.
    """
