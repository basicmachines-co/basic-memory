...
class ProjectContextMiddleware(Middleware):
    ...
    def __init__(self):
        ...
        self.default_project = ProjectItem(name=default_project_name, path=default_project_path, is_default=True)
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next) -> Any:
        ...
        if context.fastmcp_context:
            ...
            if project != active_project:
                ...
                logger.debug(f"project session saved: session_id={session_id}, active_project={active_project}")
                self.sessions[session_id] = active_project
