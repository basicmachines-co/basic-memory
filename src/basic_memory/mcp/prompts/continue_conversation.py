...
async def continue_conversation(
    ...
    """
    with logfire.span("Continuing conversation", topic=topic, timeframe=timeframe):  # pyright: ignore
        logger.info(f"Continuing conversation, topic: {topic}, timeframe: {timeframe}")
        ...
        for primary in context.primary_results:
            if hasattr(primary, "permalink") and primary.permalink not in added_permalinks:
                ...
                if hasattr(primary, "created_at"):
                    summary.append(
                        f"- **Created**: {primary.created_at.strftime('%Y-%m-%d %H:%M')}"
                    )

                summary.append("")
                summary.append(
                    f'You can read this document with: `read_note("{primary.permalink}")`'
                )
                summary.append("")
