...
async def test_move_note_success(app, client, test_project):
    ...
    content = await read_note.fn(test_project.name, "target/moved-note")
...
async def test_move_note_with_folder_creation(client, test_project):
    ...
    content = await read_note.fn(test_project.name, "deeply/nested/folder/deep-note")
...
Some additional content.
    ...
    content = await read_note.fn(test_project.name, "target/moved-complex")
...
async def test_move_note_by_title(client, test_project):
    ...
    # Verify note exists at new location
    content = await read_note.fn(test_project.name, "target/moved-by-title")
...
async def test_move_note_by_file_path(client, test_project):
    ...
    # Verify note exists at new location
    content = await read_note.fn(test_project.name, "target/moved-by-path")
...
async def test_move_note_rename_only(client, test_project):
    ...
    try:
        await read_note.fn(test_project.name, "test/original-name")
        assert False, "Original note should not exist after rename"
    except Exception:
        pass  # Expected

    # Verify new name exists with same content
    content = await read_note.fn(test_project.name, "test/new-name")
...
async def test_move_note_complex_filename(client, test_project):
    ...
    content = await read_note.fn(test_project.name, "archive/2025/meetings/meeting-notes-2025")
...
async def test_move_note_with_tags(app, client, test_project):
    ...
    content = await read_note.fn(test_project.name, "target/moved-tagged-note")
...
async def test_move_note_identifier_variations(client, test_project):
    ...
    content = await read_note.fn(test_project.name, "moved/test-document")
...
async def test_move_note_preserves_frontmatter(app, client, test_project):
    ...
    content = await read_note.fn(test_project.name, "target/moved-custom-note")
    ...
    async def test_move_note_exception_handling(self):
        ...
        with patch("basic_memory.mcp.tools.move_note.get_active_project") as mock_get_project:
            ...
            with patch(
                ...
                result = await move_note.fn("test-project", "test-note", "target/file.md")
