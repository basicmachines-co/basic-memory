...
def mock_project_list():
    ...
    return ProjectList(
        projects=[
            ...
            ProjectItem(
                name="work-notes", path="/path/to/work", is_default=False, is_current=False
            ),
            ProjectItem(
                name="personal", path="/path/to/personal", is_default=False, is_current=False
            ),
        ],
...
def mock_project_info():
    ...
    return {
        ...
        "activity": {"recently_created": [], "recently_updated": [], "monthly_growth": {}},
        "system": {
            "version": "0.13.0",
...
class TestSwitchProject:
    ...
    async def test_switch_project_info_unavailable(self, mock_project_list):
        ...
        with patch("basic_memory.mcp.tools.project_management.call_get") as mock_call:
            ...
            mock_response = AsyncMock()
            mock_response.json = Mock(return_value=mock_project_list.model_dump())
            mock_call.side_effect = [mock_response, Exception("Project info unavailable")]
