"""Structured coding-session context for Codex checkpoints."""
...
def test_coding_profile_writes_required_git_and_pull_request_frontmatter(
    ...
    assert metadata["repo"] == "basicmachines-co/basic-memory"
    ...
    assert metadata["pr"] == "#1124"
    assert metadata["pr_state"] == "open"
    assert metadata["pr_base"] == "main"
    assert metadata["pr_head"] == "feature"
...
def test_coding_profile_recall_prioritizes_current_repository() -> None:
    calls: list[dict[str, object]] = []

    async def fake_query(project_ref: str | None, **filters: object) -> dict:
        calls.append({"project_ref": project_ref, **filters})
        note_types = filters.get("note_types")
        if note_types == ["codex_session"]:
            return {"results": [{"title": "Current repo", "permalink": "sessions/current"}]}
        if note_types == ["session"]:
            return {"results": [{"title": "Legacy fallback", "permalink": "sessions/legacy"}]}
        return {"results": []}

    profile = hook_module.PROFILES[hook_module.Harness.codex]
    with patch.object(hook_module, "_query", side_effect=fake_query):
        context = hook_module.run_with_cleanup(
    ...
    repository_call = next(call for call in calls if call.get("note_types") == ["codex_session"])
    assert repository_call["metadata_filters"] == {"repo": "basicmachines-co/basic-memory"}
    assert [row["title"] for row in hook_module._rows(context.sessions)] == [
        "Current repo",
        "Legacy fallback",
    ]
