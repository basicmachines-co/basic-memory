...
def test_filter_changes_valid_path(watch_service, test_config):
    ...
    # Regular file path
    assert watch_service.filter_changes(
        Change.added, 
        str(test_config.home / "valid_file.txt")
    ) is True
    
    # Nested path
    assert watch_service.filter_changes(
        Change.added, 
        str(test_config.home / "nested" / "valid_file.txt")
    ) is True
...
def test_filter_changes_hidden_path(watch_service, test_config):
    ...
    assert watch_service.filter_changes(
        Change.added, 
        str(test_config.home / ".hidden_file.txt")
    ) is False
    
    # File in hidden directory
    assert watch_service.filter_changes(
        Change.added, 
        str(test_config.home / ".hidden_dir" / "file.txt")
    ) is False
    
    # Deeply nested hidden directory
    assert watch_service.filter_changes(
        Change.added, 
        str(test_config.home / "valid" / ".hidden" / "file.txt")
    ) is False
