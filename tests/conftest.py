...
def app_config(project_config: ProjectConfig) -> BasicMemoryConfig:
    ...
    basic_memory_app_config.default_project = project_config.name
...
@pytest_asyncio.fixture(scope="function")
async def test_project(project_config, project_repository: ProjectRepository) -> Project:
    ...
    logger.info(f"Created test project with permalink: {project.permalink}, path: {project.path}")
    return project
